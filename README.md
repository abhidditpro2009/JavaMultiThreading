...
You can create threads in Java either by extending the Thread class or by implementing a runnable interface.
As you can only extend a class once, the runnable interface implementation is the preferred method.

How do you create a thread with a Runnable interface ?
...
How do we achieve synchronization between threads trying to access shared resources ?
...
We must design a mechanism which allows a thread to obtain a lock while entering a critical section. All other threads need to be blocked. The thread in the critical section must release the monitor on exit.
...
There can be a possibility that a notify() is called before a wait(). In that case the waiting thread will never receive a notify signal and will be starved.
...
Spin locks are used to prevent spurious wake-ups. Due to the use of while loop we check again after a wakeup if the condition of lock is set to false.
If it's a spurious wakeup then the condition would be true.
...
What is the same threaded system ?
